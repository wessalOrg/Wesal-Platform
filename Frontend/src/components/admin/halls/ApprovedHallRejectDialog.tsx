...
type ApprovedHallRejectDialogProps = {
  open: boolean;
  busy?: boolean;
  onClose: () => void;
  onConfirm: () => void;
...
export default function ApprovedHallRejectDialog({
  open,
  busy = false,
  onClose,
  onConfirm,
}: ApprovedHallRejectDialogProps) {
  const t = useT();
  const titleId = useId();
  ...
  return (
    <div
      ...
      role="presentation"
      data-testid="admin-hall-reject-confirm-dialog"
    >
      <button
        type="button"
        ...
        >
          ...
          <div className="space-y-5 px-5 py-5">
            <p className="text-sm leading-7 text-[var(--wesal-text)]">
              {t("admin.reject.conflictMessage")}
            </p>
            <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
              <button
                ...
                disabled={busy}
                data-testid="admin-hall-reject-cancel"
                onClick={() => {
                  if (!busy) onClose();
                }}
              >
              ...
              <button
                ...
                data-testid="admin-hall-reject-confirm"
                onClick={() => {
                  if (!busy) onConfirm();
                }}
              >
                {busy ? t("admin.reject.confirming") : t("admin.reject.confirm")}
              </button>
            </div>
          </div>
        </div>
