...
public class RegistrationFinalizationShould : IDisposable
{
    ...
    [Fact]
    public async Task RegisteringUsers_WithoutPhone_BothSucceed()
    {
        var r1 = new RegisterRequest("User1", "nophone3@example.com", "Password123!", "Password123!", ApplicationRoles.RegisteredUser);
        var first = await _authService.RegisterAsync(r1);
        Assert.NotNull(first);

        var r2 = new RegisterRequest("User2", "nophone4@example.com", "Password123!", "Password123!", ApplicationRoles.HallOwner);
        var second = await _authService.RegisterAsync(r2);
