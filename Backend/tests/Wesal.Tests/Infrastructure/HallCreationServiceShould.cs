...
public class HallCreationServiceShould : IDisposable
{
    ...
    private class TestHallRepository : Wesal.Application.Common.Interfaces.Persistence.IHallRepository
    {
        ...
        public Task<IReadOnlyList<Wesal.Domain.Entities.Hall>> SearchApprovedHallsAsync(string? name, HallRegion? region, string? area, string? detailedAddress, DateOnly? date, TimeOnly? startTime, int skip, int take, CancellationToken cancellationToken = default) => Task.FromResult<IReadOnlyList<Wesal.Domain.Entities.Hall>>(_ctx.Halls.Skip(skip).Take(take).ToList());
